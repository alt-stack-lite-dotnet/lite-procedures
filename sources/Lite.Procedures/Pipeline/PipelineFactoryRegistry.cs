...
namespace Lite.Procedures
{
    ...
    public static class PipelineFactoryRegistry
    {
        ...
        internal static bool TryGet(Type procedureType, out IProcedurePipelineFactory factory)
            => _factories.TryGetValue(procedureType, out factory!);

        internal static bool Contains(Type procedureType) => _factories.ContainsKey(procedureType);

        internal static IReadOnlyCollection<Type> ProcedureTypes => _factories.Keys.ToArray();
